...
const diffMoment = hotelRequest => moment().diff(moment(hotelRequest.triprequest.requestDate), 'hours') as number;
...
export async function resendOfferCustomer(req: any, res: express.Response) {
  try {
    ...
    await Promise.all(hotelRequests.map(async hotelRequest => {
      const emailHR = await db.HotelRequest.findByPk(hotelRequest.id, options());
      if (!emailHR) {
        res.send({ ok: true });
      }
      const diffHoursBetweenRequestDateAndToday: number = diffMoment(hotelRequest);
      ...
      if (lastAcceptedMail) {
        const lastDeliveredMailDiffHours: number = diffMoment(hotelRequest);
